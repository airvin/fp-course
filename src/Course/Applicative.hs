...
instance Applicative List where
  ...
  (<*>) ::
    ...
    -> List b
  -- (<*>) Nil _ = Nil
  -- (<*>) (f :. fs) lista = (f <$> lista) ++ (fs <*> lista)
  -- (<*>) fs as = foldRight (\f acc -> (f <$> as) ++ acc) Nil fs
  -- (<*>) fs as = foldRight (\f -> (++) (f <$> as)) Nil fs
  -- (<*>) fs as = foldRight (\f -> (++) ((<$> as) f)) Nil fs
...
lift3 ::
  ...
  -> k c
  -> k d
lift3 fabcd ka kb kc = fabcd <$> ka <*> kb <*> kc
...
lift4 ::
  ...
  -> k d
  -> k e
lift4 fabcde ka kb kc kd = fabcde <$> ka <*> kb <*> kc <*> kd
...
filtering ::
  ...
  -> List a
  -> k (List a)
filtering p as = foldRight (\a kas -> lift2 (\b xs -> if b then a :. xs else xs) (p a) kas) (pure Nil) as 
